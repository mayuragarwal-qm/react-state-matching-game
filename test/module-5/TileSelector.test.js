...
const MockConsumer = (props) => (
  <div id="MockConsumer">
    {props.children({ numTiles: 99, handleNumTileChange: mockHandler })}
  </div>
);
...
describe("TileSelector", () => {
  ...
  it("instantiates context consumer @context-tile-selector", () => {
    ...
    expect(
      contextConsumer.exists(),
      "Did you instantiate the GameContext.Consumer?"
    ).toBeTruthy();
    expect(
      contextConsumer.children(),
      "Did you wrap the right content in the Consumer?"
    ).toHaveLength(1);

    const newWrapper = mount(<TileSelector />);

    expect(
      newWrapper.find(".tileSelectorDropdown").text(),
      "Did you remember to use the argument instead of props for numTiles?"
    ).toContain("99");

    newWrapper.find(".number").at(0).props().onClick();
    expect(
      mockHandler,
      "Did you remember to use the argument instead of props for handleNumTileChange?"
