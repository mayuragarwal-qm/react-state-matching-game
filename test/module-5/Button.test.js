...
describe("Button", () => {
  ...
  it("instantiates context consumer @context-button", () => {
    ...
    expect(
      ...
      "Did you wrap the button in the Consumer?"
    ).toHaveLength(1);
    ...
    expect(
      newWrapper.text(),
      "Did you convert the playing prop to use the context argument?"
    ).toContain("reset");

    newWrapper.find("button").props().onClick();
    expect(
      mockHandler,
      "Did you convert the startGame prop to use the context argument?"
    ).toHaveBeenCalled();
  });
});
